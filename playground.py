    ...
    while True:
        ...
        if not _gps_status.get('gps_ready'):
            ...
            if _gps_qual == 0:
                ...
                if _mode_fix_type == 1:
                    glo_gps_clr = [color_red] * _sat_count_div2 + [[0,0,0]]* (len(glo_gps_loc)-_sat_count_div2)
                elif _mode_fix_type == 2:
                    glo_gps_clr = [color_orange] * _sat_count_div2 + [[0,0,0]]* (len(glo_gps_loc)-_sat_count_div2)
                else:
                    glo_gps_clr = [color_blue] * _sat_count_div2 + [[0,0,0]]* (len(glo_gps_loc)-_sat_count_div2)
            elif _gps_qual == 2:
                glo_gps_clr = [color_green] * _sat_count_div2 + [[0,0,0]]* (len(glo_gps_loc)-_sat_count_div2)
            else:
...
def gps_pixel_plotter():
    ...
    while True:
        ...
        for i in range(len(glo_gps_loc)):
            sense.set_pixel(glo_gps_loc[i][0], glo_gps_loc[i][1], glo_gps_clr[i])
        time.sleep(glo_gps_interval)
        for i in range(len(glo_gps_loc)):
